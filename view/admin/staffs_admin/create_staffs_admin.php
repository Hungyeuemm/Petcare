...
<body>
                            ...
                            <div class="card">
                                ...
                                <div class="container mt-4">
                                    ...
                                    <div class="card-body" style="display: block;">
                                        <h5 class="card-title mb-3 text-primary">Thêm nhân viên / bác sĩ</h5>
                                        ...
                                        <form method="POST" action="../../../handle/staffs_admin_process.php">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="text" name="staff_name" class="form-control" placeholder="Nhập họ và tên" required>
                                            </div>

                                            <div class="mb-3">
                                                <label class="form-label">Số điện thoại</label>
                                                <input type="text" name="phone_staff" class="form-control" placeholder="Nhập số điện thoại">
                                            </div>

                                            <div class="mb-3">
                                                <label class="form-label">Chức vụ</label>
                                                <input type="text" name="position" class="form-control" placeholder="VD: Bác sĩ chính, Trợ lý...">
                                            </div>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select name="idrole" class="form-select" required>
                                                    ...
                                                    <?php foreach ($roles as $r): ?>
                                                        <option value="<?= $r['idrole'] ?>"><?= htmlspecialchars($r['role_name']) ?></option>
                                                    <?php endforeach; ?>
                                                </select>
                                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const petSelect = document.querySelector('#pet_id');
            const staffSelect = document.querySelector('#staff_id');
            const serviceSelect = document.querySelector('#service_id');

            customerSelect.addEventListener('change', () => {
                const id = customerSelect.value;
                fetch(`/Baitaplon/handle/datlich_ajax.php?action=pets_by_customer&customer_id=${id}`)
                    .then(res => res.json())
                    .then(data => {
                        petSelect.innerHTML = '<option value="">--Chọn thú cưng--</option>';
                        data.forEach(p => {
                            petSelect.innerHTML += `<option value="${p.id}">${p.name}</option>`;
                        });
                    });
            });
            ...
            staffSelect.addEventListener('change', () => {
                ...
                fetch(`/Baitaplon/handle/datlich_ajax.php?action=services_by_staff&staff_id=${id}`)
                    .then(res => res.json())
                    .then(data => {
                        serviceSelect.innerHTML = '<option value="">--Chọn dịch vụ--</option>';
                        data.forEach(s => {
                            serviceSelect.innerHTML += `<option value="${s.idservices}">${s.services_name}</option>`;
                        });
                    });
            });
        });
